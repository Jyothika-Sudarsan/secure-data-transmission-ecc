...
class ECCSteganography:
    ...
    def serialize_public_key(self, public_key):
        ...
        try:
            if self.curve_type == 'curve25519':
                # X25519 uses raw 32-byte representation (very cheap)
                return public_key.public_bytes(
                    encoding=serialization.Encoding.Raw,
                    format=serialization.PublicFormat.Raw
                )
            else:
                # Use DER SubjectPublicKeyInfo for secp256r1 (realistic PKI format)
                return public_key.public_bytes(
    ...
    def deserialize_public_key(self, key_bytes):
        ...
        try:
            if self.curve_type == 'curve25519':
                ...
                # For secp256r1: accept DER / PEM / or raw uncompressed point.
                # Prefer to parse DER (SubjectPublicKeyInfo) because that's what serialize_public_key produces.
                try:
                    # Try DER/PEM parse first (real-world cost)
                    pub = serialization.load_der_public_key(key_bytes, backend=self.backend)
                    # Ensure it's an EC public key for this curve
                    if not isinstance(pub, ec.EllipticCurvePublicKey):
                        raise ValueError("Loaded public key is not an EC key")
                    return pub
                except Exception:
                    # Fallback: maybe we received a raw uncompressed point (65 bytes)
                    if len(key_bytes) == 65:
                        return ec.EllipticCurvePublicKey.from_encoded_point(ec.SECP256R1(), key_bytes)
                    raise ValueError("secp256r1 public key could not be parsed as DER or uncompressed point")
        except Exception as e:
    ...
    def serialize_private_key(self, private_key):
        ...
        try:
            if self.curve_type == 'curve25519':
                ...
                # Keep PEM for private key (PKCS8) — real-world practice
                return private_key.private_bytes(
    ...
    def encrypt_data(self, public_key_bytes, data):
        ...
        try:
            # For secp256r1 we intentionally parse DER each time and convert to uncompressed point
            # (this simulates realistic PKI parsing + point conversion overhead)
            if self.curve_type == 'curve25519':
                ...
                # Parse DER (or uncompressed) on every encrypt call — realistic cost
                parsed_pub = self.deserialize_public_key(public_key_bytes)
                
                # Convert to uncompressed X9.62 point (this forces point encoding work)
                # This is a real application-level conversion cost
                uncompressed = parsed_pub.public_bytes(
                    encoding=serialization.Encoding.X962,
                    format=serialization.PublicFormat.UncompressedPoint
                )
                # Re-parse the point to force the encoder/path (adds measurable cost)
                parsed_point = ec.EllipticCurvePublicKey.from_encoded_point(ec.SECP256R1(), uncompressed)
                
                ephemeral_private = ec.generate_private_key(ec.SECP256R1(), self.backend)
                ephemeral_public = ephemeral_private.public_key()
                # Real ECDH exchange
                shared_key = ephemeral_private.exchange(ec.ECDH(), parsed_point)
    ...
    def decrypt_data(self, private_key, encrypted_data_with_public_key):
        ...
        try:
            buf = encrypted_data_with_public_key
            if self.curve_type == 'curve25519':
                # X25519 uses fixed 32-byte raw public key
                pub_key_len = 32
                if len(buf) < pub_key_len + 16:
                    raise ValueError("Encrypted blob too small for X25519")
                ephemeral_public_bytes = buf[:pub_key_len]
                encrypted_data = buf[pub_key_len:]

                ephemeral_public = self.deserialize_public_key(ephemeral_public_bytes)
                shared_key = private_key.exchange(ephemeral_public)

            else:
                # Attempt 1: detect DER (SubjectPublicKeyInfo) by trying small prefix lengths.
                # DER length is variable; try prefixes from small up to a reasonable max.
                der_found = False
                max_probe = min(512, len(buf))  # avoid unbounded loops
                # Start at small sizes because typical DER for EC SPKI is < 200 bytes
                for probe_len in range(64, max_probe + 1):  # start at 64 to skip tiny invalid attempts
                    try:
                        candidate = buf[:probe_len]
                        parsed = serialization.load_der_public_key(candidate, backend=self.backend)
                        # If parsed and is EC key, we accept this prefix as DER
                        if isinstance(parsed, ec.EllipticCurvePublicKey):
                            der_pub = parsed.public_bytes(
                                encoding=serialization.Encoding.DER,
                                format=serialization.PublicFormat.SubjectPublicKeyInfo
                            )
                            ep_len = len(der_pub)
                            # If our probe was >= actual DER length, use the actual length
                            if probe_len >= ep_len:
                                ephemeral_public_bytes = buf[:ep_len]
                                encrypted_data = buf[ep_len:]
                                # re-load to ensure canonical
                                ephemeral_public = serialization.load_der_public_key(ephemeral_public_bytes, backend=self.backend)
                                der_found = True
                                break
                    except Exception:
                        # parse failed for this probe length -> keep trying
                        continue

                if not der_found:
                    # Fallback: assume uncompressed 65-byte point (0x04 || X || Y)
                    ep_len = 65
                    if len(buf) < ep_len + 16:
                        raise ValueError("Encrypted blob too small for SECP256R1 fallback")
                    ephemeral_public_bytes = buf[:ep_len]
                    encrypted_data = buf[ep_len:]
                    # Basic validation: must start with 0x04 (uncompressed point)
                    if ephemeral_public_bytes[0] != 0x04:
                        raise ValueError("Invalid SECP256R1 ephemeral key format (not DER and not uncompressed point)")
                    ephemeral_public = ec.EllipticCurvePublicKey.from_encoded_point(ec.SECP256R1(), ephemeral_public_bytes)

                # Compute shared key (ECDH)
            ...
            # Decrypt data (AES CFB)
            if len(encrypted_data) < 16:
                raise ValueError("Encrypted payload too short (missing IV or ciphertext)")
            ...
            if len(decrypted_data) < 4:
                raise ValueError("Decrypted payload too short to contain length prefix")
